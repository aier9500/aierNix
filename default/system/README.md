...
> `hardware-configuration.nix` is **not** stored here — it is per-device and gitignored. It lives at `../hardware-configuration.nix` and is imported directly by `../configuration.nix`. See the project root README for how to supply it per machine.
...
| Per-device input | [`../hardware-configuration.nix`](../hardware-configuration.nix) — gitignored, generated by `nixos-generate-config` |
