...
| L2 | GRUB (not systemd-boot) | EFI + OS prober for dual-boot; Catppuccin GRUB theme |
...
| Declarative Kando config (config.json, menus.json) | Deferred | Config generated by GUI; sourcing from tuxies-wiki resources deferred |
| Declarative Solaar rules (rules.yaml) | Deferred | Same reason |
